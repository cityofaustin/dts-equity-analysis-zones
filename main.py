...
def transform(census_data):
    ...
    avg_inc = census_data[census_data["median_hh_inc"] != -666666666][
        "median_hh_inc"
    ].mean()
    census_data["median_hh_inc"] = census_data["median_hh_inc"].replace(
        -666666666, avg_inc
    )
    ...
    census_data["pct_wfh"] = (
        census_data["tot_wfh"].divide(census_data["B08101_001E"])
    ).fillna(0)
    ...
    census_data["pct_less_vehicles"] = (
        census_data["tot_less_vehicle"]
        .divide((census_data["B08203_001E"] - census_data["B08203_007E"]))
        .fillna(0)
    )
    ...
    census_data["pct_with_disability"] = census_data["tot_with_disability"].divide(
        census_data["tot_pop"]
    )
